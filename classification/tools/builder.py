...
def build_opti_sche(base_model, config):
    ...
    model_parameters = filter(lambda p: p.requires_grad,base_model.parameters())
    ...
    if opti_config.type == 'AdamW':
        def add_weight_decay(model, weight_decay=1e-5, skip_list=()):
            ...
            for name, param in model.named_parameters():
                # frozen the param.requires_grad = False
    ...
    # base_model.parameters()
        ...
        optimizer = optim.Adam(model_parameters, **opti_config.kwargs)
    elif opti_config.type == 'SGD':
        optimizer = optim.SGD(model_parameters, nesterov=True, **opti_config.kwargs)
    else:
