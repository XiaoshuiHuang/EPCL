...
from utils.get_clip_text_emb import get_text_emd
import time
import os
...
clip_texts_emb_path = './clip_texts_emb.npy'
if not os.path.exists(clip_texts_emb_path):
    get_text_emd()
clip_texts_emb = np.load(clip_texts_emb_path)  # [40, 512]
clip_texts_emb = torch.from_numpy(clip_texts_emb).to(torch.float).cuda()[None, :, :]
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            # [B,1024,3]
            token, logits_head = base_model(points)    # [B, 512]
            logits_clip = (token[:, None, :] @ clip_texts_emb.repeat([token.shape[0], 1, 1]).transpose(2, 1)).squeeze(1)
    
            loss_head, acc = base_model.get_loss_acc(logits_head, label)
            loss_clip, _ = base_model.get_loss_acc(logits_clip, label)

            loss = loss_head + loss_clip
        ...
        builder.save_checkpoint(base_model, optimizer, epoch, metrics, best_metrics, 'ckpt-last', args, logger = logger)         # if (config.max_epoch - epoch) < 10:
...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            _, logits_head = base_model(points)    # [B, 512]
            target = label.view(-1)

            pred = logits_head.argmax(-1).view(-1)
...
def validate_vote(base_model, test_dataloader, epoch, val_writer, args, config, logger = None, times = 20):
...
def test(base_model, test_dataloader, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            cls_tok, logits = base_model(points)
            target = label.view(-1)
