        ...
        if pxo2 is None:
            ...
            for i in range(o.shape[0]):
                if i == 0:
                    ...
                    s_i, e_i, cnt = o[i-1], o[i], o[i] - o[i-1]
                x_b = x[s_i:e_i, :]
                x_b = torch.cat((x_b, self.linear2(x_b.sum(0, True) / cnt).repeat(cnt, 1)), 1) # dim=1024
                x_tmp.append(x_b)
            x = torch.cat(x_tmp, 0)
            ...
            x2_knn_prob_shrink = rearrange(
                x2_knn_prob_flatten_shrink, '(m k) 1 -> m k 1', k=self.nsample) # (m, nsample, 1)
...
class FixedSymmetricTransitionDownBlock(nn.Module):
    def __init__(self, in_planes, out_planes, stride, nsample):
        super().__init__()
        self.stride, self.nsample = stride, nsample
    ...
    def forward(self, pxo):
        ...
        if self.stride != 1: # downsampling
            ...
            for idx in n_o:
                ...
                lens.append(size)
            num_token = min(lens)
            n_o = [num_token * i for i in range(1, len(n_o) + 1)]
            n_o = torch.cuda.IntTensor(n_o)
            # ===================================================
            idx = pointops.furthestsampling(p, o, n_o)  # (m)
            n_p = p[idx.long(), :]  # (m, 3)
